...
const CourseCards: React.FC = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await dispatch(getAllCourseAction({isPublished:true}));
      ...
      <div className='flex justify-between '>
       ...
       <details className="dropdown dropdown-right ">
        ...
        {/* <div className="dropdown dropdown-bottom dropdown-end mr-[15rem]">
          ...
          <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-white dark:bg-gray-700 rounded-box w-52">
            <li>
              <details className="dropdown dropdown-right">
                <summary className="px-2 py-1 font-semibold">Category</summary>
                <ul className="menu p-2 bg-white dark:bg-gray-700 rounded-box">
                  {category.map((category: any, index: any) => (
                    <li key={index}>
                      <div className="form-control">
                        <label htmlFor={`category${index}`} className="label cursor-pointer">
