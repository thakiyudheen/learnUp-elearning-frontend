...
console.log(error)
  useEffect(() => {
    const fetchInstructors = async () => {
      try {
        const instructorData = await dispatch(getAllInstructorsAction({ page: currentPage, limit: itemsPerPage }));
        if (getAllInstructorsAction.fulfilled.match(instructorData)) {
          setInstructors(instructorData.payload.data.instructors);
          setTotalPages(Math.ceil((instructorData.payload.data.totalItems / itemsPerPage)))
        } else {
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      try {
        const instructorData = await dispatch(getRequestAction({ page: currentPage, limit: itemsPerPage }));
        if (getRequestAction.fulfilled.match(instructorData)) {
          setRequests(instructorData.payload.data.requested.filter((el: any) => !el.isVerified && !el.isRejected));
          setReject(instructorData.payload.data.requested.filter((el: any) => el?.isReject));
          setTotalPages(Math.ceil((instructorData.payload.data.totalItems / itemsPerPage)))
          // setInstructors(instructorData.payload.data.requested.filter((el:any) => el.isVerified));
    ...
    {isBLockModal&& <ConfirmModal message={"Action"} onCancel={onCancel} onConfirm={handleBlock}/>}
      ...
      <div className="flex ml-[4rem] mb-4">
        <div role="tablist" className="tabs tabs-boxed bg-gray-200 dark:bg-gray-700">
          ...
          <a role="tab" onClick={() => { setActiveTab('requests'); setCurrentPage(1); }} className={`tab ${activeTab === 'requests' && 'tab-active'}`}>Requests</a>
        </div>
      </div>
                ...
                <tbody>
                  {instructors.map((instructor, index) => (
                    <tr key={index} className="text-center border-b">
                      ...
                      <td className="px-4 py-2 border">
                        <button
                          className={`border font-bold py-1 px-2 rounded-lg ${instructor.isBlocked
                            ? 'hover:bg-green-600 hover:text-white border-green-600 text-green-600'
                            : 'hover:bg-red-600 hover:text-white border-red-600 text-red-600'
                            }`}
